...
def auth(app):
    ...
    def register(): # 注册接口
        ...
        if(len(db_setting.my_db(sql))!=0):#判断用户是否已注册
            ...
            sql1 = "INSERT into user_account(account,password,create_time,update_time) values('%s','%s','%s','%s')" % (
            account, pwd, create_time, create_time)
            ...
            sql4 = "INSERT INTO `user_avatar_image` (`user_id`,create_time) VALUES ('%s','%s')" % (
    ...
    def send_code():  # 检查账户是否注册，发送验证码
        account = request.json.get('account')
        option_action = request.json.get('option_action')#为1代表注册，2时代表修改密码
        sql = "SELECT id from user_account where account='%s' " % (account)  # 查询用户是否注册过
        if(option_action==str(1)):
            if (len(db_setting.my_db(sql)) == 0):  # 判断用户是否未注册
                update_code = random.randint(100000, 999999)  # 随机生成6位模拟验证码校验
                create_time = datetime.utcnow()
                sql2 = "INSERT INTO `update_code` (`account`, `update_code`, `create_time`,code_status) VALUES ('%s', '%s', '%s',0)" % (
                account, update_code, create_time)
    ...
    def check_code():  # 检查验证码是否有效
        ...
        option_action = request.json.get('option_action')#为1代表注册，2时代表修改密码
        if(option_action==str(1)):
            sql = "SELECT id from user_account where account='%s' " % (account)  # 查询用户是否未注册
            if (len(db_setting.my_db(sql)) == 0):  # 判断用户是否未注册
                # userid = db_setting.my_db(sql)[0][0]
                sql2 = "select user_id,update_code,create_time from(SELECT * from update_code where account='%s' ORDER BY create_time DESC limit 1)as a where code_status=0" % (
                    account)
                ...
                if (db_setting.my_db(sql2)):
                    ...
                    if (check_code == str(update_code)):
                        if (time_now > code_time + timedelta(minutes=5)):
                            ...
                            sql3 = "UPDATE `update_code` SET  `code_status` = 1 WHERE `user_id` ='%s'  AND `update_code` = '%s'; " % (
                                account, update_code)
                            db_setting.my_db(sql3)
            ...
            if (len(db_setting.my_db(sql)) != 0):  # 判断用户是否已注册
                userid = db_setting.my_db(sql)[0][0]
                sql2 = "select user_id,update_code,create_time from(SELECT * from update_code where user_id='%s' ORDER BY create_time DESC limit 1)as a where code_status=0" % (
                    userid)
                time_now = datetime.utcnow()
                if (db_setting.my_db(sql2)):
                    update_code = db_setting.my_db(sql2)[0][1]  # 验证码\
